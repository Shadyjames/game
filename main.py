...
class App:
    def __init__(self):
        ...
        self.world.set(world.Tile(5), 9, 9)
        self.world.set(world.Tile(5), 9, 7)
        self.world.set(world.Tile(5), 8, 8)
        #world.load()
...
class Game(App):
    def __init__(self):
        ...
        self.player.image = pygame.transform.scale(pygame.image.load("assets/images/player.png").convert(32), (int(self.tilewidth * self.player.size), int(self.tilewidth * self.player.size)))
    def main(self):
        while self.running:
            ...
            if self.player.image:
                player_location = (world_render_rect[0] + world_render_rect[2] / 2.0 - self.player.size * self.tilewidth / 2.0, 
                                   world_render_rect[1] + world_render_rect[3] / 2.0 - self.player.size * self.tilewidth / 2.0)
                screen.blit(self.player.image, player_location)
